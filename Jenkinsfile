     ...
     steps {
       script {
         openshift.withCluster() {
           ...
           apply = openshift.apply(openshift.raw("new-app ${REPO} --name=${APP_NAME} -l app=${APP_NAME} --strategy=source --as-deployment-config=true --dry-run --output=yaml").actions[0].out)  
